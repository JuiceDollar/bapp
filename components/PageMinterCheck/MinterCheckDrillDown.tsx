...
export default function MinterCheckDrillDown() {
	...
	const juiceDollarAddress = ADDRESS[WAGMI_CHAIN.id].juiceDollar;
	...
	const bridges = useMemo(() => {
		const ONE_JUSD = 10n ** 18n;
		return allBridges.filter((b) => b.minted >= ONE_JUSD);
	}, [allBridges]);
		...
		<AppCard>
			...
			>
				<div className="flex items-center gap-2">
					...
					<a
						href={`${explorerUrl}/token/${juiceDollarAddress}`}
						target="_blank"
						rel="noopener noreferrer"
						className="text-sm text-link hover:underline"
						onClick={(e) => e.stopPropagation()}
					>
						{shortenAddress(juiceDollarAddress)}
					</a>
				</div>
				<div className="flex items-center gap-2">
				...
				<div className="overflow-hidden">
					<div className="flex flex-col gap-2 pt-3">
						...
						<div>
									...
									<div className="flex items-center gap-2">
										...
										<FontAwesomeIcon
											...
											className={`w-3 h-3 transition-transform duration-300 ${
												expandedCategory === "positions" ? "rotate-180" : ""
											}`}
										/>
									</div>
								</div>
								<div className="w-full h-1.5 rounded-full bg-slate-200 overflow-hidden">
									<div className="h-full rounded-full bg-blue-500 transition-all" style={{ width: `${pctPositions}%` }} />
								</div>
							</button>
							...
							<div
								className={`grid transition-all duration-300 ${
									expandedCategory === "positions" ? "grid-rows-[1fr]" : "grid-rows-[0fr]"
								}`}
							>
								<div className="overflow-hidden">
																	...
																	return (
																		...
																		>
																			<div className="flex items-center justify-between">
																				...
																				<span className="text-text-muted">
																					{formatCurrency(minted, 0, 0)} {TOKEN_SYMBOL} (
																					{pctPos.toFixed(1)}%)
																				</span>
																			</div>
						...
						<div>
								...
								<div className="w-full flex items-center justify-between">
									...
									<div className="flex items-center gap-2">
										...
										<FontAwesomeIcon
											icon={faChevronDown}
											className={`w-3 h-3 transition-transform duration-300 ${
												expandedCategory === "bridges" ? "rotate-180" : ""
											}`}
										/>
									</div>
								</div>
							...
							<div
								className={`grid transition-all duration-300 ${
									expandedCategory === "bridges" ? "grid-rows-[1fr]" : "grid-rows-[0fr]"
								}`}
							>
								<div className="overflow-hidden">
									<div className="flex flex-col gap-1 pl-5 pt-1">
												...
												<div key={group.symbol}>
													...
													>
														<div className="w-full flex items-center justify-between">
															...
															<div className="flex items-center gap-2">
																<span className="text-text-muted">
																	{formatCurrency(group.totalMinted, 0, 0)} {TOKEN_SYMBOL} (
																	{pctOfTotal.toFixed(1)}%)
																</span>
																<FontAwesomeIcon
																	icon={faChevronDown}
